	...
	metabasetest.Run(t, func(ctx *testcontext.Context, t *testing.T, db *metabase.DB) {
		...
		t.Run("invalid EncryptedMetadata", func(t *testing.T) {
			...
			for i, scenario := range metabasetest.InvalidEncryptedUserDataScenarios() {
				...
				// InvalidEncryptedUserDataScenarios produces a scenario wherein the checksum algorithm
				// is set and the encrypted checksum is unset. This scenario is valid for BeginObject,
				// so skip it.
				if userData.Checksum.Algorithm != storj.ObjectChecksumAlgorithmNone && userData.Checksum.EncryptedValue == nil {
					continue
				}
